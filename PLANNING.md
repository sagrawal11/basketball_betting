...
- [x] **7.3 — Eliminate full-season team-rating leakage.** ✅ Done. `_merge_team_context` previously merged `TEAM_*`/`OPP_*` on `(team, SEASON)` from full-season aggregates, so every mid-season row saw end-of-season (future) ratings. Added `_load_team_ratings_timeline()` which builds season-to-date (cumulative-through-game, within team×season) ratings from the game-level Kaggle data, and rewrote `_merge_team_context` to attach them with `merge_asof(direction="backward", allow_exact_matches=False)` — the same prior-only pattern as `KG_PREV_*`. Proven by `backend/tests/test_team_ratings_leakage.py` (RED: mid-season row returned the full-season aggregate 107.2; GREEN: returns prior-only 91.0, first-of-season is NaN). Verified end-to-end on real data (261/261 ratings populated, prior-only). *Note: `all_team_stats`/`_load_team_stats`/`self.team_stats` are now unused by the engine — left in place (README drift tracked by 12.8); a retrain is still pending per the agreed sequence.*
- [x] **7.4 — Make `position_encoded` a stable, shared mapping.** ✅ Done. `_prepare_raw` used `pd.Categorical(df["position"]).codes`, whose integers depend on the positions present in the current frame, so the same position mapped to different codes across players and between train/inference. Added the fixed `POSITION_GROUP_ENCODING = {"G": 0, "F": 1, "C": 2}` and now derive `position_encoded` from the normalized `POSITION_GROUP` via that dict (default branch included). Proven by `backend/tests/test_position_encoding.py` (RED: "PG" encoded differently across frames / group G mapped to 1 vs default 0; GREEN: stable, 1:1 with POSITION_GROUP). Verified on real data. *Retrain still pending per the agreed sequence.* *Severity: medium (skew).*
- [x] **7.5 — Make archetype (`ARCH_PROB_*`) columns deterministically present.** ✅ Done. Two silent `except` blocks let the `ARCH_PROB_*` columns appear in one path but vanish in another (skew; `train_hybrid.py` even branches on `ARCH_PROB_0`). Fix: `ArchetypeEngine.transform_frame` now always emits a fixed `ARCH_PROB_0..N-1` schema (+ `ARCH_ARGMAX`), filling the GMM training prior (`gmm.weights_`, exposed via new `n_archetypes`/`prior_`) instead of dropping columns or using 0.0 when probabilities can't be computed; `_build_from_dataframe` no longer swallows the transform; and `default_engine` fails loudly when a present archetype model can't load (absence is still allowed for the bootstrap phase). Proven by `backend/tests/test_archetype_determinism.py` (RED: simulated predict failure raised/dropped columns and corrupt-model load was swallowed; GREEN: prior-filled fixed schema, loud raise). Verified on real data (10 cols, sum to 1.0). *Retrain still pending per the agreed sequence.* *Severity: high (skew).*
- [x] **7.6 — Add a train-serve skew guard at inference.** ✅ Done. `predict_player_stats` reindexed the inference row to `feature_columns` and filled missing/NaN with 0.0 at `logger.debug` only — invisible skew, and it didn't even count present-but-NaN columns. Added `NBAPredictor._feature_skew_report` (counts both absent AND NaN/non-numeric managed columns that get 0.0-filled, tagging high-signal `ARCH_PROB_*`/`OPP_PRIOR_AVG_*`/`KG_PREV_*`/`KG_OPP_PREV_*`/`DVP_*` via `SKEW_SENSITIVE_PREFIXES`) and `_check_feature_skew` which emits a structured `WARNING` once the filled fraction crosses `SKEW_WARN_FRACTION` (0.20); wired into the inference path. Proven by `backend/tests/test_inference_skew_guard.py` (RED: no diagnostic/guard existed; GREEN: correct counts + warning fires only when non-trivial). *Severity: high.*
- [x] **7.7 — Fix the home/away rolling cross-contamination.** ✅ Done. The old `merge_asof`-on-date approach joined the latest prior HOME-rolled AND AWAY-rolled values onto *every* row, so a home game carried away-form and vice-versa (and same-day rows were collapsed by `drop_duplicates`). Rewrote `_add_home_away_rolling_features` to roll each venue's games within that venue (shift→rolling) and attach the result only to that venue's rows; the opposite venue's columns stay NaN, and the full `HA_*` schema is always created. Proven by `backend/tests/test_home_away_rolling.py` (RED: away game carried `HA_HOME_PTS_L5=10` borrowed from home games; GREEN: venue-isolated). Verified on real data (no opposite-venue values ever populated). *Severity: medium.*
