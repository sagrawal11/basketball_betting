...
- [ ] **7.2 — Remove leaky raw columns from the global trainer.** Because of 7.1, `train_global.py:70-77` can admit same-game outcome columns (`FGA`, `FTA`, `REB`, `FG_PCT`, raw `TS_PCT`/`USG_PCT`/`*_PER100`, `PLUS_MINUS`) as features. After 7.1, assert the selected columns are a subset of the managed whitelist and fail loudly if not. *Severity: critical.*
