#!/usr/bin/env python3
"""
Train global LightGBM regressors (one per target) on stacked player feature rows.
Uses chronological train/test split and saves artifacts under models/artifacts/global/.
"""
...
from config.paths import ARTIFACTS_DIR, GLOBAL_MODEL_DIR, PLAYER_DATA_DIR
...
def main() -> None:
    ...
    for target in TARGET_STATS:
        ...
        try:
            y = full[target]
            model, m = train_one_target(X_frame[feature_cols], y, seasons)
            path = args.out / f"global_lgbm_{target}.joblib"
            joblib.dump({"model": model, "target": target, "metrics": m}, path)
            metrics_all[target] = m
            logger.info("Trained %s Final MAE=%.3f", target, m.get("final_mae", m.get("mae", 0.0)))
        except Exception as e:
            logger.exception("Failed training %s: %s", target, e)

    with open(args.out / "global_training_metrics.json", "w", encoding="utf-8") as f:
